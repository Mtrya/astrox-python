...
`access.compute(...)` does not accept a separate `constraints=` argument. Constraints are participant metadata, so they travel with the entity through `FromObjectPath` and `ToObjectPath`. The SDK rejects raw constraint dictionaries at the entity boundary; use `components.elevation_constraint(...)`, `components.range_constraint(...)`, or `components.az_el_mask_constraint(...)`.
...
`participants` accepts `components.Entity` and `components.EntityGroup` values. `start_participant`, `end_participant`, and `access.connection(...)` endpoints accept an `Entity`, an `EntityGroup`, or a string name. String names are forwarded as references; the SDK does not locally check that a string appears in `participants`.
