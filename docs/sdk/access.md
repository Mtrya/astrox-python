...
Access functions compute line-of-sight style access between named ASTROX analysis objects. The SDK assembles the Python request payload from `components.Entity`, `components.EntityGroup`, and `access.Connection` values, then returns the ASTROX JSON-like response dictionary unchanged.
...
`from_entity` and `to_entity` accept `components.Entity` values. Name strings and entity groups are chain concepts and are not accepted by `compute(...)`.
...
`access.compute(...)` does not accept a separate `constraints=` argument. Constraints are participant metadata, so they travel with the entity through `FromObjectPath` and `ToObjectPath`. The SDK rejects raw constraint dictionaries at the entity boundary; use `components.elevation_constraint(...)`, `components.range_constraint(...)`, `components.az_el_mask_constraint(...)`, `components.sun_exclusion_angle_constraint(...)`, or `components.moon_exclusion_angle_constraint(...)`.

The supported constraint types are elevation-angle limits in degrees, range limits in kilometers, azimuth/elevation mask samples in radians, and Sun/Moon exclusion-angle minimums in degrees. Cross-validation in `tests/validation/cross_validation/access/test_compute_constraints_skyfield.py` establishes the following for representative fixed-site to SGP4 and SGP4-to-fixed-site cases:
...
Cross-validation in `tests/validation/cross_validation/access/test_compute_exclusion_constraints_invariants.py` covers `SunExclusionAngle` and `MoonExclusionAngle` on the access surface, including direct `access.chain(...)` routes for the calibrated direct-link cases. In validated fixed-site cases, including the constrained site as either `from_entity` or `to_entity`, the constraint is satisfied when the Sun/Moon is below the constrained site's apparent local horizon, or when the target line of sight is separated from the topocentric astrometric Sun/Moon vector by at least `MinimumValue` degrees. In validated SGP4 cases, including constrained SGP4 as `from_entity` and an access-producing no-drag SGP4 `to_entity` fixture, the constraint is satisfied when Earth occults the Sun/Moon from the satellite, or when the target line of sight is separated from the satellite-observer astrometric Sun/Moon vector by at least `MinimumValue` degrees. `minimum_deg=0` preserves the unconstrained intervals; higher Sun/Moon thresholds narrow or split the interval set, so validation compares the full interval set rather than pass count. Drag-bearing TLE fixtures can still surface live SGP4 propagation errors before returning intervals; those errors are treated as propagation failures, not as ExclusionAngle interval semantics.
...
`participants` accepts `components.Entity` and `components.EntityGroup` values. Entity constraints, including Sun/Moon exclusion constraints, are embedded in participant definitions. `start_participant`, `end_participant`, and `access.connection(...)` endpoints accept an `Entity`, an `EntityGroup`, or a string name. String names are forwarded as references; the SDK does not locally check that a string appears in `participants`.
...
Access workflows accept named `components.Entity` values. The maintained direct-access contract cases cover fixed sites, SGP4 TLE positions, central bodies, HPOP positions, CZML-like sampled positions, simple ascent positions, and ballistic positions. Semantic cross-validation currently covers fixed-site to SGP4, SGP4 to fixed-site, blocked ground-to-ground, SGP4 to J2, and HPOP/two-body mixed-model cases.
