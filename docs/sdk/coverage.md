...
For validated static cases, `response_time.by_grid_point(..., compute_type="Maximum")` returns the maximum uncovered gap duration, including gaps at the start or end of the analysis window. `response_time.by_grid_point(..., compute_type="Minimum")` returns `0` for grid points that are covered at least once in the representative intermittent case; in a never-covered case, both `Minimum` and `Maximum` return the full analysis-window duration. The response-time at-time and over-time routes are exposed because ASTROX exposes them. Validated continuous-coverage cases return `0`; validated intermittent and never-covered cases currently return HTTP 500 server errors, and the SDK preserves those errors.
