...
`include_asset_access_results=True` returns per-grid-point, per-asset intervals. Validated cases show those intervals compose back into `SatisfactionIntervalsWithNumberOfAssets`, and duplicate identical assets are preserved as separate asset entries. For representative SGP4 satellites over surface `lat_lon_grid(...)` fixtures, those per-asset intervals match independent Skyfield line-of-sight/topocentric-horizon calculations within the calibrated live validation tolerance.
...
`grid_point_constraints` accepts shared `components.Constraint` values such as `components.elevation_constraint(...)`, `components.range_constraint(...)`, `components.sun_exclusion_angle_constraint(...)`, and `components.moon_exclusion_angle_constraint(...)`. Validated permissive range/elevation/Sun/Moon-exclusion constraints preserve the unconstrained intervals, and validated restrictive range/elevation/Sun/Moon-exclusion constraints return interval subsets. For Sun/Moon grid-point exclusions, `tests/validation/cross_validation/coverage/test_exclusion_grid_constraints_skyfield.py` compares representative coverage intervals against the same independent body-separation convention as access, using the grid point as the constrained observer. Over-restrictive representative constraints currently return a server worker error instead of empty no-access intervals. `components.az_el_mask_constraint(...)` is accepted by the SDK, but ASTROX currently rejects it in this coverage role with a server message that the current object is not a ground-station object.
...
Validated FOM cases currently show these ASTROX conventions. `simple_coverage.by_grid_point(...)` returns `1` for a grid point covered at least once during the analysis window and `0` otherwise; the at-time variant returns current coverage at `time`. `coverage_time.by_grid_point(..., compute_type="TotalTimeAbove")` returns total covered duration in seconds. `number_of_assets.by_grid_point(...)` supports `Average`, `Maximum`, and `Minimum` over the asset-count trace; the at-time variant returns the current count. FOM `grid_stats(...)` routes use simple arithmetic minimum, maximum, and average over the returned grid-point values, not grid weights; this is validated on a representative grid where the arithmetic and weighted averages differ. `grid_stats_over_time(...)` routes sample from `start` through `stop` at `step_s` increments and match the corresponding at-time route statistics for the validated simple-coverage, number-of-assets, and revisit-time cases.

For validated static cases, `response_time.by_grid_point(..., compute_type="Maximum")` returns the maximum uncovered gap duration, including gaps at the start or end of the analysis window. `response_time.by_grid_point(..., compute_type="Minimum")` returns `0` for grid points that are covered at least once in the representative intermittent case; in a never-covered case, both `Minimum` and `Maximum` return the full analysis-window duration. For validated at-time cases before a later access, `response_time.by_grid_point_at_time(...)` returns the remaining time until the next access; during access it returns `0`. The response-time over-time route is exposed because ASTROX exposes it, but validated intermittent and never-covered cases currently return HTTP 500 server errors when the sample series includes a grid point with no later access inside the analysis window. The SDK preserves those errors.

For validated cases, `revisit_time.by_grid_point(...)` returns uncovered-gap duration statistics for `Average`, `Maximum`, and `Minimum`, including boundary gaps. In a never-covered case, all three compute types return the full analysis-window duration. In a continuously covered case, they return `0`. `revisit_time.by_grid_point_at_time(...)` returns `0` when the point is covered at `time`; when the point is uncovered, it returns the whole containing uncovered gap duration, not just the remaining time until the next access.

For duplicate two-asset cases and offset two-asset cases, FOM resource-count behavior follows the same convention as `coverage.compute(...)`: `minimum_assets` and the currently observed `exactly_assets` behavior act as thresholds for whether a point is covered, while `number_of_assets` FOM values still report the actual active asset count. Unsupported `compute_type` strings are rejected by ASTROX for the metric routes that expose `ComputeType`; they are not ignored, defaulted, or silently remapped.

Validated FOM cases consume `grid_point_sensor`, `elevation_constraint`, `range_constraint`, `sun_exclusion_angle_constraint`, and `moon_exclusion_angle_constraint` consistently with `coverage.compute(...)`: the same local interval/gap derivations from modified `ComputeCoverage` traces reproduce representative simple-coverage, coverage-time, number-of-assets, response-time, and revisit-time FOM values. `AzElMask` currently rejects consistently in representative FOM routes and `coverage.compute(...)` for the coverage grid-point role. At-time FOM routes use ASTROX's internal transition precision. The interval strings returned by `coverage.compute(...)` are rounded to milliseconds, so a returned transition timestamp can be a fraction of a millisecond before the internal transition used by at-time FOM routes.

## Validation Scope

SDK behavior tests cover exact request lowering, optional-key omission, type rejection, public imports, and raw response pass-through. Live snapshots cover representative callability and response shape for grid generation, compute, elevation/range grid-point constraints, the two non-FOM reports, and all FOM routes.

Validation currently verifies representative `LatLonBounds`, `LatitudeBounds`, and `Global` grid point centers and cell boundaries, representative `CbLatLonBounds` box-tiling invariants, the invariant that `ComputeCoverage` with `include_coverage_points=True` echoes the same point ordering as `GetGridPoints`, representative SGP4-to-grid membership against independent Skyfield line-of-sight geometry, resource-count composition from per-asset intervals, ASTROX `ExactlyN` threshold behavior in duplicate two-asset compute and FOM cases, grid-point sensor and range/elevation interval-filter invariants, Sun/Moon grid-point exclusion intervals against independent Skyfield body-separation geometry, weighted percent-coverage samples, one-asset coverage-by-asset summaries, FOM interval/gap conventions, FOM no-coverage and continuous-coverage edge cases, FOM two-asset resource-count cases, representative FOM grid-branch behavior for all four grid constructors, FOM modifier consumption, FOM AzElMask rejection in the coverage grid-point role, at-time boundary behavior, successful FOM response-shape keys, and metric-specific `compute_type` rejection. The exact `CbLatLonBounds` count rule, fixed-site assets, and the current response-time dynamic-route HTTP 500 behavior for intermittent and never-covered cases remain caveated.
