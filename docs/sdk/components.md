...
- `components.sun_exclusion_angle_constraint(minimum_deg=..., text=...)` lowers to an ASTROX `SunExclusionAngle` constraint. `minimum_deg` is omitted unless supplied.
- `components.moon_exclusion_angle_constraint(minimum_deg=..., text=...)` lowers to an ASTROX `MoonExclusionAngle` constraint. `minimum_deg` is omitted unless supplied.
...
Current cross-validation in `tests/validation/cross_validation/access/test_compute_constraints_skyfield.py` calibrates elevation, range, and AzElMask constraints against independent Skyfield/WGS84 topocentric geometry. Verified behavior for representative fixed-site to SGP4 and SGP4-to-fixed-site cases includes elevation and range limits in the participant's local topocentric frame, geometric range evaluation even with light-time delay enabled, intersection semantics for multiple constraints and both-participant constraints, evaluation of satellite-side elevation and range constraints in the satellite's Earth-fixed geodetic local frame, piecewise-linear AzEl mask interpolation in azimuth with 0/360 closure, and the site-only validity of AzEl masks. `AzElMask.MaxRange` is forwarded but not enforced by ASTROX. Supplying `minimum > maximum` while `maximum_enabled=True` raises a server validation error; ordered but impossible thresholds return empty passes.

`tests/validation/cross_validation/access/test_compute_exclusion_constraints_invariants.py` and `tests/validation/cross_validation/coverage/test_exclusion_grid_constraints_skyfield.py` cover the newer Sun/Moon exclusion constraints. Validated fixed-site access, direct-chain, and coverage grid-point cases show `minimum_deg=0` preserves the unconstrained intervals, while larger Sun/Moon thresholds can narrow or split intervals. The calibrated fixed-observer convention is: the constraint is satisfied when the Sun/Moon is below the constrained observer's apparent local horizon, or when the target line of sight is separated from the topocentric astrometric Sun/Moon vector by at least `MinimumValue` degrees. For constrained SGP4 participants, including SGP4 `from_entity` cases and an access-producing no-drag SGP4 `to_entity` case, the constraint is satisfied when Earth occults the Sun/Moon from the satellite, or when the target line of sight is separated from the satellite-observer astrometric Sun/Moon vector by at least `MinimumValue` degrees. FOM routes consume Sun/Moon exclusion constraints consistently with the modified `ComputeCoverage` traces. Drag-bearing TLE fixtures can still surface live SGP4 propagation errors before returning intervals; those errors are treated as propagation failures, not as ExclusionAngle interval semantics.
