...
| `targets` | — | 目标列表，`orbits.Tle` 序列；省略时 SDK 不会把 `Targets` 字段发往 ASTROX |
...
`targets` 省略或为 `None` 时，SDK 会按约定省略请求中的 `Targets` 字段，保留服务器的默认行为；按 OpenAPI/服务器契约，此时服务器会从服务器卫星目录加载全部目标参与计算，目标数量很大时可能触发大规模计算。建议调用者显式传入目标列表，把计算范围限定在所需目标上。
...
| `collision_probability` | `float` | 服务端返回的标量；当前只能按服务端标量使用，不能赋予统计学碰撞概率语义 |
