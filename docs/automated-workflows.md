...
- regenerate `openapi/fixtures/STATUS.md` while the transitional fixture inventory remains checked in
...
The OpenAPI drift workflow does not run fixture replay, fixture reconciliation, discovery backlog reporting, pytest, package build checks, live smoke checks, or SDK semantic validation. Normal PR CI is the merge gate for refresh PRs.
...
If the stable live OpenAPI document matches `openapi/astrox.openapi.yaml`, the workflow exits successfully without opening a PR, replaying fixtures, running tests, or creating an issue.
...
Fixtures are reference-only discovery evidence and test-seed material. They are not a scheduled drift gate.

`openapi/fixtures/STATUS.md` is regenerated by OpenAPI drift while it remains checked in, because OpenAPI schema changes can make the generated inventory stale. This is temporary maintenance of the existing fixture mechanism, not an endorsement of fixture replay as a long-term automation layer.

The long-term direction is to promote useful fixture coverage into tests that protect supported SDK behavior, then retire fixture branches, `STATUS.md`, and fixture-specific scripts that no longer provide useful reference value.

## Automation Principles

- Keep workflow responsibilities separate.
- Prefer one live check path for supported SDK behavior instead of duplicate fixture replay plus tests.
