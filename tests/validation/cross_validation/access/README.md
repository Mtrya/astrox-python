...
Endpoint option behavior, HPOP/two-body model-pair probes, and chain topology edge cases that compare ASTROX only with ASTROX now live under `tests/validation/live_snapshot/access/`.
...
| HPOP/two-body site companion, distinct-orbit, near-coincident, and coincident satellite-pair behavior | live snapshot only |
