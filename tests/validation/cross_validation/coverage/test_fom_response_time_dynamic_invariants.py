...
#     - ResponseTime ValueByGridPointAtTime after final access: verified as null when no later access exists inside the analysis window
#     - ResponseTime GridStatsOverTime for intermittent coverage: partial; verified to return nullable statistics without an HTTP error, while mixed-null aggregation remains unresolved
...
#     - Datas[].FOM_Value: verified against a local remaining-time-to-next-access derivation, including null when no later access exists
#   Parameters:
#     - time: verified before access, during access, mixed before/during access, after final access, and outside the analysis window
#     - step_s: representative nullable GridStatsOverTime output is recorded in live snapshots
#     - minimum_assets/exactly_assets: covered by resource-options FOM tests for static routes; the representative dynamic case uses minimum_assets=1
#   Comparison:
#     - External: local interval/gap derivation from ComputeCoverage SatisfactionIntervalsWithNumberOfAssets
#     - Constants: no physical constants; nullability is derived from whether an uncovered point has a later positive interval inside the analysis window
...
#     - ResponseTime returns null for an uncovered point with no later access inside the analysis window; this differs from RevisitTime, which returns the containing gap duration.
...
@pytest.mark.parametrize("seconds", [0.0, 30.0, 300.0, 600.0, 1740.0])
...
def remaining_time_to_next_access(intervals: list[dict], seconds: float) -> float | None:
...
def run_all_checks() -> int:
    for seconds in [0.0, 30.0, 300.0, 600.0, 1740.0]:
